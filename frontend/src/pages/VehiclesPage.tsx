      ...
      <div className="animate-fade-in">
        ...
        <div className="flex items-center justify-between mb-10">
          ...
          {isManager && (
            <button
              ...
              className="inline-flex items-center gap-2.5 px-6 py-3 bg-amber-500 hover:bg-amber-400 text-slate-900 font-semibold rounded-xl text-sm transition-colors cursor-pointer whitespace-nowrap"
            >
        ...
        <div className="relative mb-8">
          <input
            ref={searchInputRef}
            type="text"
            placeholder="Search by year, make, or model (e.g. 2014 Ford, Charger, 201)..."
            value={search}
          ...
          {search && (
            <button
              onClick={() => setSearch('')}
              className="absolute right-5 top-1/2 -translate-y-1/2 p-1 text-slate-500 hover:text-white transition-colors cursor-pointer"
            >
              <X className="w-4 h-4" />
            </button>
          )}
        ...
        <div className="bg-slate-900 border border-slate-800 rounded-2xl overflow-hidden">
          {loading ? (
            ...
            <div className="p-16 text-center">
              ...
              <h3 className="text-lg font-semibold text-slate-400 mb-1">No vehicles found</h3>
              <p className="text-sm text-slate-600">{search ? 'Try a different search' : 'Add vehicles to get started'}</p>
            </div>
          ) : (
            <>
                ...
                <table className="w-full">
                  <thead>
                    <tr className="border-b border-slate-800">
                      <th className="px-8 py-5 text-left text-xs font-semibold text-slate-500 uppercase tracking-wider">Year</th>
                    ...
                    {vehicles.map((v) => (
                      <tr key={v.id} className="hover:bg-slate-800/30 transition-colors">
                        <td className="px-8 py-4">
                          <span className="inline-flex px-4 py-1.5 bg-amber-500/10 text-amber-400 text-xs font-mono font-semibold rounded-lg">
                            {v.year}
                          </span>
                        </td>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm" onClick={() => setShowModal(false)}>
          <div className="bg-slate-900 border border-slate-800 rounded-2xl w-full max-w-lg mx-4 shadow-2xl animate-fade-in" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center justify-between px-7 py-5 border-b border-slate-800">
              ...
              <button onClick={() => setShowModal(false)} className="p-1.5 text-slate-500 hover:text-white transition-colors cursor-pointer">
                <X className="w-5 h-5" />
              </button>
            </div>
