      ...
      <div className="animate-fade-in">
        <div className="flex items-center justify-between mb-10">
          <div>
            ...
            <p className="text-sm text-slate-500 mt-2">Create and manage part requests</p>
          </div>
        ...
        <div className="bg-slate-900 border border-slate-800 rounded-2xl overflow-hidden">
          {loading ? <div className="p-16 text-center text-slate-500">Loading...</div> : requests.length === 0 ? (
                ...
                <tbody className="divide-y divide-slate-800/50">
                  {requests.map((req) => (
                    <tr key={req.id} className="hover:bg-slate-800/50 transition-colors">
                      <td className="px-8 py-4 text-sm font-bold text-white">#{req.id}</td>
                      <td className="px-8 py-4"><span className={`inline-flex px-2.5 py-1 text-xs font-semibold rounded-md ${statusBadge[req.status] || ''}`}>{req.status}</span></td>
                      ...
                      <td className="px-8 py-4 text-sm text-slate-400">{req.creator?.name}</td>
                      <td className="px-8 py-4 text-xs text-slate-400 whitespace-nowrap">{formatDate(req.createdAt)}</td>
                      <td className="px-8 py-4 text-sm text-slate-400">{req.notes || '—'}</td>
                      <td className="px-8 py-4">
                        <div className="flex gap-2">
                          {req.status === 'PENDING' && isManager && (
                            <button onClick={() => handleApprove(req.id)} className="inline-flex items-center gap-3 px-5 py-2.5 text-xs font-medium bg-emerald-500/10 text-emerald-400 rounded-lg hover:bg-emerald-500/20 whitespace-nowrap transition-colors cursor-pointer"><Check className="w-3.5 h-3.5" /> Approve</button>
                          )}
      ...
      {showCreateModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm" onClick={() => setShowCreateModal(false)}>
          <div className="bg-slate-900 border border-slate-800 rounded-2xl w-full max-w-2xl mx-4 shadow-2xl animate-fade-in" onClick={(e) => e.stopPropagation()}>
            ...
            <form onSubmit={handleCreateRequest} className="px-7 py-6 space-y-4">
              <div>
              ...
              <div>
                <label className="block text-xs font-medium text-slate-400 mb-2 uppercase tracking-wider">Notes (optional)</label>
              ...
              <div className="flex justify-end gap-3 pt-4 border-t border-slate-800">
                <button type="button" onClick={() => setShowCreateModal(false)} className="inline-flex items-center gap-3 px-6 py-3.5 bg-slate-800 border border-slate-700 rounded-xl text-sm text-slate-300 hover:text-white whitespace-nowrap transition-colors cursor-pointer">Cancel</button>
                <button type="submit" className="inline-flex items-center gap-3 px-7 py-3.5 bg-amber-500 hover:bg-amber-400 text-slate-900 font-semibold rounded-xl text-sm whitespace-nowrap transition-colors cursor-pointer">Create Request</button>
              </div>
            </form>
          </div>
        </div>
